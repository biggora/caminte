...
Schema.prototype.define = function defineClass(className, properties, settings) {
    ...
    if(typeof this.adapter.autoupdateModel == "function"){
        this.adapter.autoupdateModel(className, function(err){
            if(err){
                schema.emit('error', className);
            } else {
                schema.emit('newModel', className);
            }
        });
    } else {
        schema.emit('newModel', className);
