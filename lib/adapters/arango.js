...
Arango.prototype.parseConds = (key, conds, bindVars, props, self, ct) => {
    ...
    Object.keys(val).forEach((condType) => {
        let sqlCond = '';
        //let _val = val[condType];
        let _val = self.toDatabase(props[key], val[condType], true); // -- Quando inserir no banco colocar essa
...
Arango.prototype.buildFilter = (conds, adapter, model, bindVars = {}, or = false) => {
    let query = '', ct = 0,
        self  = adapter,
        ...
        if(key === 'or') {
            if((conds[key] instanceof Array)){
                conds[key].forEach((val, index) => {                    
                    ...
                    query+=self.buildFilter(val, adapter, model, bindVars, true).query;
                    if(ct > 0)
                        query+=")";
                    ct++;
                });
                ...
                query+=self.buildFilter(conds[key], adapter, model, bindVars, true).query;
            ...
            if(conds[key] instanceof Object){
                query+= self.parseConds(key, conds, bindVars, props, self, bvLen)
            } else {
                query+= "x.`"+key+"` == @values"+bvLen; 
                bindVars["values"+bvLen] = self.toDatabase(props[key], conds[key], true);
            }
            ct++;
        }        
    });
...
Arango.prototype.all = Arango.prototype.find = function (model, filter, callback) {
    ...
    if (filter) {
        if (filter.where && Object.keys(filter.where).length > 0) {
            var csql = self.buildFilter(filter.where, self, model);
            query.push(csql.query);
