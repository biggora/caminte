...
Arango.prototype.fromDatabaseP = function (props, data){
    if (!data)
        return null;
...
Arango.prototype.buildWhere = (conds, adapter, model, bindVars = {}, or = false) => {
    let query = '', ct = 0,
        self  = adapter,
    ...
    Object.keys(conds).forEach((key) => {
        if(key === 'or') {
            if((conds[key] instanceof Array)){
                conds[key].forEach((val, index) => {                    
                    if(ct > 0){
                        query+= (or ? ' OR ' : ' AND ');
                        query+="(";
                    }
                    query+=self.buildWhere(val, adapter, model, bindVars, true).query;
                    if(ct > 0)
                        query+=")";
                    ct++;
                });
                ...
                query+=self.buildWhere(conds[key], adapter, model, bindVars, true).query;
            ...
            const bvLen = Object.keys(bindVars).length;
            if(ct > 0)
                query+= (or ? ' OR ' : ' AND ');

            if(conds[key] instanceof Object){
                query+= parseCond(key, conds, bindVars, props, self, bvLen)
            } else {
                query+= "x.`"+key+"` == @values"+bvLen; 
                bindVars["values"+bvLen] = self.toDatabase(props[key], conds[key], false);
            }
            ct++;
        }        
    });
...
Arango.prototype.query = function (filter, callback) {
    const self = this;
    let maql;

    if(!filter)
        return callback && callback(new Error('Undefined filter'));

    if(!('where' in filter))
        return callback && callback(new Error('Undefined where in filter'));

    if(typeof filter.where === typeof Object)
        filter.where = filter.where.find(' ');

    if (!Object.keys(filter.bindVars).length) {
        maql = filter.query;
    } else {
        maql = self.aqlQuery([filter.where]);
        maql.bindVars = filter.bindVars;
    }
    ...
    self.client.query(maql, function (err, cursor) {
        ...
        if('properties' in filter)
            data = data.map(function (i) {
                return self.fromDatabaseP(filter.properties, i);
            });
        return callback && callback(err, data)
    }.bind(self));
};
...
Arango.prototype.count = function (model, callback, where) {
    ...
    if (where && Object.keys(where).length > 0) {
        ...
        let query = 'FOR x IN @@collection';
        const csql = self.buildWhere(where, self, model, bindVars);
        query+= ' FILTER '+csql.query;
        query+= ' COLLECT WITH COUNT INTO length RETURN length';

        let maql = self.aqlQuery([query]);
        maql.bindVars = bindVars;
        ...
        self.client.query(maql, function(err, cursor) {
            ...
            return callback && callback(err, (cursor._result[0] || 0))
        });
    } else {
        self.collection(model).count(function (err, res) {
...
// TODO: implement
Arango.prototype.remove = function (model, filter, callback) {
    var self = this;
    if (('where' in filter) && Object.keys(filter.where).length > 0) {
        let bindVars = {
            '@collection': model
        };

        let query = 'FOR x IN @@collection';
        const csql = self.buildWhere(filter.where, self, model, bindVars);
        query+= ' FILTER '+csql.query;
        query+= ' REMOVE x IN @@collection';
        ...
        self.client.query(maql, function(err, cursor) {
            ...
            if (err) {
                ...
                _t = cursor.extra.stats.writesExecuted;
            
            return callback && callback({err: err, count : _t});
        });
    } else
        return callback && callback({err: new Error('Undefined filter'), count : 0});
};
...
var parseCond = (key, conds, bindVars, props, self, ct) => {
    if(!(conds[key] instanceof Object) || conds[key] == null) return '';

    let query = 'x.`'+key+'`';
    const val = conds[key];

    Object.keys(val).forEach((condType) => {
        let sqlCond = '';
        let _val = self.toDatabase(props[key], val[condType], true);

        if((condType === 'inq' || condType === 'nin') && _val.length === 0)
            _val = [(condType === 'inq' ? 1 : 0)];
        ...
        if(condType == 'between'){
            ...
            bindVars['whereL'+ct] = _val[0];
            bindVars['whereG'+ct] = _val[1];
        } else {
            query+=sqlCond;
            query+="@values"+ct;
            bindVars["values"+ct] = _val;
        }
