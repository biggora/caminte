...
SQLite3.prototype.alterTable = function(model, actualFields, actualIndexes, done, checkOnly) {
    var self = this;
    var m = this._models[model];
    var propNames = Object.keys(m.properties);
    var sql = [], isql = [];
    var reBuild = false, needAlter = false;
    ...
    for (var prop in m.properties) {
        if ('undefined' !== typeof m.properties[prop]['index']
            ...
            var foundKey = false;
            actualIndexes.forEach(function(index) {
    ...
    if (tSql.length) {
        if (checkOnly) {
            ...
            return this.command(tSql.join(';\n'), function(err) {
                if (reBuild) {                    
                    return rebuid(model, m.properties, actualFields, actualIndexes, done);
                } else {
                    return done(err, needAlter);
                }
            });
        }
    } else {
         return done && done(null, reBuild);
    }
