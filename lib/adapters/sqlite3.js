...
SQLite3.prototype.update = function (model, filter, data, callback) {
    ...
    self.command(sql, queryParams, function (err1) {
        self.command('SELECT changes() FROM ' + self.tableEscaped(model), {}, function (affected) {
             return callback && callback(err1, affected || 0);
        });
    });
};
