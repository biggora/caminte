...
exports.initialize = function initializeSchema(schema, callback) {
    if (typeof schema.settings.url === 'undefined') {
        schema.settings.url = 'http://'
            + schema.settings.host + ':'
            + schema.settings.port;
    }
    schema.client = new neo4j.GraphDatabase(schema.settings.url);
    schema.adapter = new Neo4j(schema.client, schema);
    schema.adapter.schema = schema;
    process.nextTick(callback);
};

function Neo4j(client, schema) {
    this.schema = schema;
    ...
    this.allNodes(model, function (err, collection) {
        ...
        if(collection && collection.length) {
            ...
            collection && collection.forEach && collection.forEach(function (node) {
                ...
                self.destroy(model, node.id, done);
            });
        } else {
            callback(null, null);
        }
...
Neo4j.prototype.remove = function remove(model, filter, callback) {
    var self = this, wait, error = null;
    self.all(model, filter, function(err, collection){
        if (err) {
...
Neo4j.prototype.updateAttributes = function updateAttributes(model, id, data, cb) {
    data.id = id;
    this.node(id, function (err, node) {
        this.save(model, helpers.merge(node.data, data), cb);
    }.bind(this));
};
